<script>
    ...
    $: fileExplorerText = modes[selectedMode].path || "";

    function handleFileChange(file) {
        if (!file) return;
        const reader = new FileReader();
        reader.onload = (e) => dispatch("fileLoaded", e.target.result);
    ...
    function copyPath() {
        if (fileExplorerText) {
            navigator.clipboard.writeText(fileExplorerText).then(() => {
                ...
                timeout = setTimeout(() => (showPopover = false), 500);
            });
        }
    }

    function openSteamCloud() {
        window.open(modes.steamcloud.redirect, "_blank");
    }
</script>

<div class="w-full max-w-2xl mx-auto space-y-4">
    <div class="flex flex-wrap justify-center gap-2">
        {#each Object.keys(modes).filter((mode) => mode !== "steamcloud") as mode}
            <button
                class="px-4 py-2 rounded-lg border transition hover:bg-gray-200 {selectedMode ===
                mode
    ...
    <div
        class="text-center mb-3 border rounded-lg px-6 py-3 transition transform hover:scale-105 active:scale-95 relative flex flex-wrap items-center gap-3"
    >
        <!-- allow shrinking + wrapping -->
        <button
            on:click={copyPath}
            class="flex-1 min-w-0 text-center whitespace-normal break-words break-all px-2 py-2 bg-transparent text-sm"
        >
            {fileExplorerText}
        </button>

        {#if selectedMode === "steam"}
            <a
                href="https://store.steampowered.com/account/remotestorageapp/?appid=1030300"
                target="_blank"
                rel="noopener noreferrer"
                class="shrink-0 text-blue-400 hover:text-blue-300 text-xs underline whitespace-nowrap transition"
                title="Open Steam Cloud Saves"
            >
                Steam Cloud
            </a>
        {/if}
    ...
    {#if !modes[selectedMode].redirect}
        ...
        >
            <p class="mb-3">
                Drag & drop your .dat file here, or click below to upload
            </p>
